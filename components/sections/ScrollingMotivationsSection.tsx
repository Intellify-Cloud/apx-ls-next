          ...
          <div className="flex animate-scroll items-center gap-[24px]">
            {duplicatedMotivations.map((keyword, index) => (
              <React.Fragment key={index}>
                <span className="font-display text-[14px] leading-[14px] font-semibold text-[#ed5c2c] uppercase tracking-widest whitespace-nowrap">
                  {keyword}
                </span>
                {index < duplicatedMotivations.length - 1 && (
                  <span className="text-[#ed5c2c]/40 mx-[4px]">•</span>
                )}
              </React.Fragment>
            ))}
          </div>
